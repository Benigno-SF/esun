<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Wfo_TrackView.aspx.cs" Inherits="SFC_WEB_APP.Mod_Agro.Wfo_TrackView" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="Body" runat="server">
    <div class="row">
        ...
        <div class="col-md-10">
            <div id="myDiv" class="map col-md-12"></div>
            <div class="controlsx" style="display: none">
                <button class="btn btn-sm btn-secondary" id="play">Iniciar</button>
                <button class="btn btn-sm btn-secondary" id="pause">Pausar</button>
                <button class="btn btn-sm btn-secondary" id="reset">Resetear</button>
                <button class="btn btn-sm btn-secondary" id="next">Siguiente</button>
                <button class="btn btn-sm btn-secondary" id="prev">Anterior</button>
               
                <button class="btn btn-sm btn-secondary" id="slow">Lento</button>
                <span class="btn btn-sm btn-white black" id="speed">speed: 1x</span>
                <button class="btn btn-sm btn-secondary" id="fast">Rapido</button>
            </div>
        </div>
    </div>
    <script>
        ...
        var name = "";
            $("#btnBusc").click(function (event) {
                ...
                $('#myDiv').html("<img src='../Img/load_map.gif' width='90'><br /><b>Trazando Ruta ...</b>"); 
            var obj = new Object();
            obj.vcIdEmpresa = fuGetCdEmpre();
            obj.vcIdSucursal = $('#<%=ddlIdFundo.ClientID%>').val();
            obj.vnIdDispositivo = $('#<%=ddlnIdDisp.ClientID%>').val();
            obj.vcFechaIni = $('#<%=txtFeInic.ClientID %>').val();
            obj.vcFechaFin = $('#<%=txtFeFina.ClientID %>').val();
            if (obj.vnIdFundo != '' && obj.vnIdDispositivo != '' && obj.vcFechaIni != '' && obj.vcFechaFin != '') {
                $.ajax({
                    type: 'POST',
