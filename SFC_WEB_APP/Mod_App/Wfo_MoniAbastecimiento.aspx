...
<head runat="server">
    ...
    <script type="text/javascript" src="http://190.119.196.77/ch/js/Chart.min.js"></script>
</head>
<body>
            ...
            <tr>
                ...
                <td style="width: 70%; padding: 0px; margin: 0px">
                    <div class="text-center" style="font-size: 3rem; color: white">
                        <span id="dvTitle" runat="server"></span>PANEL DE ABASTECIMIENTO - <span id="lblCLiente">25-11-2020</span>
                    </div>
                </td>
            </tr>
        </table>
        <div>
            <div class="row">
                <div class="col-6" style="font-size: 16px;">
                    ...
                    <h4>Recepcionado Total</h4>
                    <table id="tblRecepcion" style="text-align: center; margin-bottom: 0px; width: 100%" class="tbRend">
                        <thead>
                            <tr>
                                <td>CLIENTE</td>
                                <td>VARIEDAD</td>
...
<%--                                                        <div class="row">
                        <div class="col-12">
                            <div class="card" style="zoom: 1;">
                                <div class="card-header">
                                    <h4 style="color: #000">Variedad Total</h4>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (i = 0; i < lst.length; ++i) {
                            ...
                            plt += lst[i].palets;
                            kgs += lst[i].kilos;
                            if (lst[i].NombreUbicacion == 'ALMACEN GENERAL' || lst[i].NombreUbicacion == 'UBICACION PROCESOS') {
        ...
        //Lo ordenas a gusto.
        $('#lblCLiente').text(d + "/" + m + "/" + y);

        function LoadDataRecepcion() {
            var obj = new Object();
            obj.vnIdMonitor = $('#<%=hdfIdMonitor.ClientID%>').val();
            obj.vcfecha = '';
            if (obj.vnIdMonitor != -1) {
                $.ajax({
                    type: 'POST',
                    url: "SerApp.asmx/ListRecepcionMercancia",
                    data: JSON.stringify({ obj: obj }),
                    dataType: 'json',
                    contentType: 'application/json; charset=utf-8',
                    success: function (data) {
                        ...
                        for (i = 0; i < lst.length; ++i) {		
                            ...
                            kgss += lst[i].kilos;
                            var nub = lst[i].NombreProveedor;
                            vvtb += "<tr><td>" + nub.substring(0, 26) + "</td><td>" + lst[i].NombreArticulo + "</td><td><i class='fas fa-cubes'></i> " + lst[i].palets + "</td><td><i class='fas fa-weight-hanging'></i> " + formatNumber.new(lst[i].kilos.toFixed(2)) + "</td></tr>";
                        }
                        $("#lbltpalets").html('<b>' + plts + '</b>');
                        $("#lbltkilos").html('<b>' + formatNumber.new(kgss.toFixed(2)) + ' Kg.</b>');
                        $("#tblRecepcion tbody").html(vvtb);
                        ...
                        //for (var i in lst) {
                        //    name.push(lst[i].NombreArticulo.substring(0, 26));
                        //    palets.push(lst[i].kilos);
                        //}
        ...
        $(document).ready(function () {
            ...
            setInterval(LoadDataRecepcion, 300000);
        });
