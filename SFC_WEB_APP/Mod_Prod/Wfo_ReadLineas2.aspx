<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Wfo_ReadLineas2.aspx.cs" Inherits="SFC_WEB_APP.Mod_Prod.Wfo_ReadLineas2" %>
                    ...
                    success: function (data) {
                        ...
                        if (lst != '') {
                            ...
                            if (area != "" && linea != "" && cliente != "" && embalaje != "" && cultivo != "" && variedad != "" && supervisor != "") {
                                ...
                                Load_Agrupo(area, linea, namelinea, ctype);
...
<%--                    obj.vnIdClieProv = $("#<%=ddlCliente.ClientID%>").val();
                    ...
                    obj.vnIdVariedad = $("#<%=ddlVariedad.ClientID%>").val();
                    obj.vnIdSupervisor = $("#<%=ddlSupervisor.ClientID%>").val();--%>
                    if (obj.vcLabor == 'C' && $('#ctype').val() != '2') {
                        ShowToast("error", 'ETIQUETA SOLO PARA TAPER CLAMSHELL...', 'Etiqueta de Taper Clamshell, verifique');
                        $(document).ready(function () {
                            var snd = new Audio("../sound/error.mp3");
                            snd.play();
                        });
                        $('#msg').text('Etiqueta de Taper Clamshell, Cambie');
                        errorcode = errorcode + 1;
                        $('#errores').text(errorcode);
                        clearTimeout(clear);
                        fclear();
                    } else if (obj.vcLabor == 'E' && $('#ctype').val() == '2') {
                        ShowToast("error", 'NO ES ETIQUETA DE CLAMSHELL...', 'Solo etiquetas Clamshell');
                        $(document).ready(function () {
                            var snd = new Audio("../sound/error.mp3");
                            snd.play();
                        });
                        $('#msg').text('NO ES ETIQUETA DE CLAMSHELL');
                        errorcode = errorcode + 1;
                        $('#errores').text(errorcode);
                        clearTimeout(clear);
                        fclear();
                    } else if (obj.vcLabor == 'S' && $('#ctype').val() == '2') {
                        ShowToast("error", 'NO ES ETIQUETA DE CLAMSHELL...', 'Solo etiquetas Clamshell');
                        $(document).ready(function () {
                            var snd = new Audio("../sound/error.mp3");
                            snd.play();
  ...
  $('body').on('change', '#<%=ddlAreaGrupo.ClientID%>', function () {
      var id = $(this).val();
      $("#ctype").val($("#<%=ddlAreaGrupo.ClientID%> option:selected").attr('cla'));

      
      Load_Embalaje($("#<%=ddlCultivo.ClientID%> option:selected").val(), 0);
        ...
        function Load_Agrupo(id, idgru, namegru, ctype) {
            var obj = new Object();
            obj.vcIdEmpresa = fuGetCdEmpre();
            obj.vnIdArea = id;
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (lst.length != '0') {
                        ...
                        for (i = 0; i < lst.length; ++i) {
                            option += '<option value="' + lst[i].nIdGrupo + '" cla="' + lst[i].nClamshell + '">' + lst[i].cDescAGrupo + '</option>';
                        }
                        if (idgru != 0) {
        ...
        function Load_Embalaje(id, idem) {
            ...
            obj.vnIdCultivo = id;
            obj.vnIdEmbalaje = '0';
            obj.vnIdTipoc = $("#<%=ddlAreaGrupo.ClientID%> option:selected").attr('cla');
            if (obj.vnIdTipoc == 'null') {
                obj.vnIdTipoc = 0;
            }
            $.ajax({
                type: 'POST',
                url: "../SerProd.asmx/EmbalajeListc",
                data: JSON.stringify({ obj: obj }),
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
                success: function (data) {
