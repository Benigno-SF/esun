<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Wfo_Table_Evento.aspx.cs" Inherits="SFC_WEB_APP.Mod_Maes.Wfo_Table_Evento" %>
...
<div class=" mb-12">
  <div class="card-header border border-3 border-success text-center"> <h5>EVENTOS DEL TABLE</h5> </div>
...
<!-- datagrid mostrar datos -->
      ...
      <div class="col-md-12">
                <div class="card">
                    <div class="card-content collapse show">
                        <div class="card-body card-dashboard" id="container-tbdist2">
                            <div id="tbOperaciones2">
...
<script>
        ...
        function TbDataGrid(data) {
            ...
            var dataGrid = $("#data_grid").dxDataGrid({
                ...
                onCellClick: function (e) {
                    ...
                    // areglos para con dtaos asignados..
                    const columnSelect_ini = [];
                    const cabecera_ini = [];
                    const data_ini = [];
                    ...
                    if (e.rowType === 'header') {
                        ...
                        function evento() {
                            /*var el = document.querySelector(".dx-datagrid-drag-header");*/
                            //// nombre de la clase.
                            //var x = $(el).attr("class");
                            //console.log("nombre de la clase :" + x);

                            // vao de dicha clase
                            //var value = $(el).text();
                            //console.log("contenido la clase :" + value);

                            //// estilos css
                            //var s = $(el).attr("style");
                            //console.log("css de la clase :" + s);

                            // agregarando la acabecera en un array ..
                            $(".dx-toolbar-before .dx-group-panel-item").each(function () {
                                /*columnSelect.push($(this).text());*/
                                columnSelect.push($(this).text());
                            });

                            // agregarando la acabecera en un array ..
                            $(".dx-header-row .dx-datagrid-action").each(function () {
                                cabecera.push($(this).text());
                            });

                            // agregarando la data en un array ..
                            $(".dx-scrollable-content td").each(function () {
                                data.push($(this).text());
                            });

                            // llenamos apotrsod so arreglos.
                            //console.log(columnSelect);
                            //console.log(cabecera);
                            //console.log(data);

                            // recorremos el arreglo
                            for (i = 0; i < columnSelect.length; i++) {
                                columnSelect_ini.push(columnSelect[i]);
                            }

                            for (i = 0; i < cabecera.length; i++) {
                                cabecera_ini.push(cabecera[i]);
                            }

                            for (i = 0; i < data.length; i++) {
                                data_ini.push(data[i]);
                            }

                            // array llenados.
                            datos(columnSelect_ini, cabecera_ini, data_ini);
                        }
                    } 
    ...
    // funcion llenaod de arreglo
    function datos(columnSelect_ini, cabecera_ini,data_ini) {

        // construimos  el json
        var objeto = {};
        var arrra_final = [];

        for (let i = 0; i < cabecera_ini.length; i++) {
            objeto.id = (i + 1);
            objeto.name = cabecera_ini[i];
            arrra_final.push({ ...objeto });
        }
        console.log(JSON.stringify(arrra_final));

        console.log(columnSelect_ini);
        console.log(cabecera_ini);
        console.log(data_ini);

        $('#tbOperaciones2').html('<div id="data_grid2"></div>');

        DevExpress.localization.locale('es');

        var dataGrid = $("#data_grid2").dxDataGrid({
            dataSource: arrra_final,
            scrolling: {
                mode: "virtual"
            },
            grouping: {
